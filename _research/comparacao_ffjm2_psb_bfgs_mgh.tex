...
unknown. BFGS and BOBYQA are both given the same box, $[0,0.5]^{n_{grid}}$,
and for the same reason, since a trial step whose direction is too large
...
\mathrm{penalty}(\xi)$, where $\mathrm{penalty}$ (weight $10^6$)
...
most 1000 evaluations of the residual, Algorithm~\ref{complexity} at
most 1000 outer iterations), and BFGS and Algorithm~\ref{complexity}
...
termination code directly, \texttt{grad\_conv} (Algorithm~\ref{complexity}
...
(Algorithm~\ref{complexity}'s $\sigma$ stuck at its ceiling) and
...
Algorithm~\ref{complexity} already the cheapest of the three. 
...
$2.5$h to reach RMSD near to $4.68\times10^{-2}$. Algorithm~\ref{complexity}'s
...
This paper describes an approach based on quartic residual models for
solving sum-of-squares problems whose objective function, and
especially its gradient, is expensive to evaluate, and whose domain
contains forbidden regions where the objective is undefined,
unreliable, or has no physical meaning, without the boundary of those
regions being known in advance.

Section~1 addresses a degeneracy at the algorithm's very first
iteration ($k=0$), where no curvature information is yet available.
The quartic subproblem collapses to a damped Gauss-Newton step there,
which we handle with a steepest-descent, Armijo-line-search safeguard.

Section~\ref{sec:computational-tests} uses the 34 MGH sum-of-squares
problems to calibrate Algorithm~\ref{complexity}, choosing the PSB
Hessian-update model and Newton as the quartic-subproblem solver for
their lower cost, and an order-2 backtracking line search for the BFGS
baseline. Compared on all 34 problems, the two methods reach
statistically indistinguishable objective values (Mann-Whitney test),
but Algorithm~\ref{complexity} is significantly cheaper, the better or
tied method on about 80\% of the problems.

Section~\ref{cap:coeficient_estimation} applies all three methods to a
real engineering problem, calibrating the Manning coefficient in the
Saint-Venant equations, across four scenarios (synthetic and real
data, dimensions 2 and 10). Algorithm~\ref{complexity} is the cheapest
and most accurate of the three in three of the four scenarios; the
exception is the synthetic dimension-10 case, where its regularization
parameter $\sigma$ stalls and it becomes the worst of the three. The
same failure mode reappears, without the same cost, in the hardest
scenario, real data at dimension 10, where Algorithm~\ref{complexity}
is once again the cheapest and most accurate of the three.

Taken together, these results suggest that the quartic-model approach
is a competitive alternative to BFGS and BOBYQA for expensive
sum-of-squares problems with forbidden regions, typically cheaper
whenever it converges, though its regularization-stall failure mode,
first seen in the dimension-10 experiments, remains an open point for
future work.
