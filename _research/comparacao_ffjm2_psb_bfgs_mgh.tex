...
\centering
% \includegraphics[width=\textwidth]{old_solver.png}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
\centering
% \includegraphics[width=\textwidth]{new_solver.png}
\end{minipage}
...
We whant to solve a coefficient estimation problem in a PDE. That is, evaluate a objective function is solve a PDE and derivate this function is derivate a PDE solver. In this paper, 
...
unknown. BFGS and BOBYQA minimize the boxed, penalized sum of squares
$\|F(\theta)\|_2^2 + \mathrm{penalty}(\theta)$ (box $[0,0.5]^{n_{grid}}$,
penalty weight $10^6$); Algorithm~\ref{complexity} minimizes the
unpenalized $f(\theta) = \frac12\|F(\theta)\|_2^2$ directly, so its $f$
column below is, on runs where the box constraint is inactive, exactly
half of the other two methods', e.g.\ real data, $n_{grid}=3$, below,
where all three reach the same minimizer up to rounding, with
$f\approx1.0085$ for BFGS/BOBYQA versus $f\approx0.5043$ for
Algorithm~\ref{complexity}. RMSD is recomputed uniformly from the raw
(unpenalized) residual, $\mathrm{RMSD}(\theta) = \|F(\theta)\|_2/\sqrt{m}$,
at each method's minimizer, so it is directly comparable across all rows
...
Algorithm~\ref{complexity} & \multicolumn{7}{l}{not available in this run} \\
\bottomrule
\end{tabular}
}%
\end{table}
...
wall-clock time to get there. Algorithm~\ref{complexity} was not run (or did not
