...
To calibrate $n_g(x)$ against this residual, four benchmark scenarios
compare the same three solvers used throughout this report --- BFGS
(backtracking, order 2), BOBYQA, and \texttt{ffjm2} --- applied directly to
$F(\eta)$ above, exactly as in Table~\ref{tab:subproblem-solvers} (each
solver stands in as the whole optimizer, not as \texttt{ffjm2}'s internal
subproblem solver). Two scenarios are \emph{twin} experiments, where the
reference $z_{obs}$ is itself generated by the model at a known
$\eta^\star$, so the true minimizer is known exactly; the other two use the
real observed data $z_{obs}$, where the true minimizer (if it exists) is
unknown. BFGS and BOBYQA minimize the boxed, penalized sum of squares
$\|F(\eta)\|^2 + \mathrm{penalty}(\eta)$ (box $[0,0.5]^{n_{grid}}$, penalty
weight $10^6$); \texttt{ffjm2} minimizes the unpenalized
$\frac12\|F(\eta)\|^2$ directly, so its $f$ column below is, on runs where
the box constraint is inactive, exactly half of the other two methods' ---
e.g.\ real dim.\ 3 below, where all three reach the same minimizer up to
rounding: $f\approx1.0085$ for BFGS/BOBYQA versus $f\approx0.5043$ for
\texttt{ffjm2}. RMSD is recomputed uniformly from the raw (unpenalized)
residual at each method's minimizer, so it is directly comparable across
all rows of every table below.

\begin{table}[htbp]
\centering
\caption{Twin experiment, dimension 2: $\eta^\star=(0.20,\,0.15)$,
    $\eta^0=(0.09,\,0.09)$.}
\label{tab:twin-dim2}
\resizebox{\textwidth}{!}{%
...
$\eta^\star=(0.20,0.15)$; \texttt{ffjm2} is the cheapest of the three by
every cost measure (fewest evaluations, least time) despite also reaching
the tightest gradient norm and RMSD.

\begin{table}[htbp]
\centering
\caption{Twin experiment, dimension 10: decreasing profile $\eta^\star$
    from $0.12$ to $0.07$, $\eta^0=(0.09,\ldots,0.09)$. (Increasing profiles
...
\noindent
At dimension 10 the gap between \texttt{ffjm2} and the other two widens
sharply: it reaches a gradient norm roughly $7$ orders of magnitude smaller
than BFGS or BOBYQA, using about $7$--$35\times$ fewer function evaluations
...
\begin{table}[htbp]
\centering
\caption{Real data, dimension 3: $\eta^0=(0.09,0.09,0.09)$.}
\label{tab:real-dim3}
\resizebox{\textwidth}{!}{%
\begin{tabular}{l r r r r r r l}
\toprule
...
\begin{table}[htbp]
\centering
\caption{Real data, dimension 10: $\eta^0=(0.09,\ldots,0.09)$.}
...
from a stationary point, despite the lower evaluation count and time of
the two completed runs. BOBYQA reports \texttt{SUCCESS} and reaches the
lowest RMSD ($4.64\times10^{-2}$), but needs $840$ evaluations and
$\approx2.9$h of wall-clock time to get there. \texttt{ffjm2} was not run
(or did not complete) for this scenario; that row is left for a future
update once \texttt{comparar\_solvers\_real\_dim10} has been re-run with it
included.
