...
vector $\xi \in \IR^{n_{grid}}$, with $\xi_i := n_g(x_i)$ for
...
the derivative of the objective with respect to $\xi$ accurately;
...
$\frac12\|F(\xi)\|_2^2 + \mathrm{penalty}(\xi)$, where $\mathrm{penalty}$
...
minimizes $f(\xi) = \frac12\|F(\xi)\|_2^2$ directly. We consider a box ther for the BFGS and BOBYQA methods to help the convergence of solver. 
...
\texttt{grad\_conv} for a genuine gradient-norm stopping-criterion match,
for either Algorithm~\ref{complexity} or BFGS,
...
$\xi_i \in [0, 0.5]$ for every $i=1,\ldots,n_{grid}$, enforced through
the additive penalty of weight $10^6$; Algorithm~\ref{complexity}
minimizes $f(\xi)$ unconstrained, since its own regularization already
...
Algorithm~\ref{complexity} & $5.86\times10^{-11}$ & $5.80\times10^{-19}$ & $6.65\times10^{-8}$ & 10 & 5 & 158.0 & \texttt{grad\_conv} \\
\bottomrule
\end{tabular}
}%
\end{table}
...
$\xi^\star=(0.20,0.15)$; Algorithm~\ref{complexity} is the cheapest of the three by
every cost measure (fewest evaluations, least time) despite also reaching
the tightest gradient norm and RMSD.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\linewidth]{plot_calor_pregerado_com_solvers.pdf}
	\caption{RMSD heatmap of the pregenerated-data experiment ($n_{grid}=2$) with the path of accepted iterates of each method overlaid: BFGS (red), BOBYQA (blue), and Algorithm~\ref{complexity} (green). The white diamond marks the shared starting point $\xi^0=(0.25,0.25)$; the star marks each method's final iterate.}
...
\caption{Pregenerated-data experiment, $n_{grid}=10$: decreasing profile $\xi^\star$
    from $0.20$ to $0.15$, $\xi^0=(0.25,\ldots,0.25)$. (Increasing profiles
    over comparable ranges were tested manually and diverge before
...
Algorithm~\ref{complexity} & $4.38\times10^{-6}$ & $3.24\times10^{-9}$ & $1.02\times10^{-3}$ & 15 & 9 & 444.3 & \texttt{grad\_conv} \\
\bottomrule
\end{tabular}
}%
\end{table}
...
\noindent
At $n_{grid}=10$ Algorithm~\ref{complexity} is now the clear winner: it is
...
(\texttt{alpha\_too\_small}, \texttt{converged=false}) far from a
stationary point, with the worst RMSD and gradient norm of the three
($\approx2.00\times10^{-1}$ and $\approx3.33\times10^2$, respectively).
...
Algorithm~\ref{complexity} already the cheapest of the three; at
$n_{grid}=10$ it remains by far the cheapest, but now it is also the only
one of the three that reaches a certified stationary point.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.8\linewidth]{plot_calor_novo.pdf}
	\caption{RMSD heatmap of the real-data experiment ($n_{grid}=2$) with the path of accepted iterates of each method overlaid: BFGS (red), BOBYQA (blue), and Algorithm~\ref{complexity} (green). The white diamond marks the shared starting point $\xi^0=(0.25,0.25)$; the star marks each method's final iterate.}
...
minimize $\tfrac12\|F(\xi)\|_2^2$). BOBYQA certifies its own
