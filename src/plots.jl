...
function le_pontos_aceitos_solvers(pontos_csv = "results/comparacao_solvers_real_dim2_pontos.csv")
    ...
    # Cabeçalho lido de volta em vez de assumido fixo: aceita tanto o formato
    # antigo (`method,point_index,x`) quanto o atual, mais rico, escrito por
    # `_comparar_solvers` (`method,point_index,function_evaluations,
    # gradient_evaluations,RMSD,x`). `x` é sempre o último campo e é o único
    # que pode conter vírgulas internas (é uma lista `[a, b, ...]`, às vezes
    # entre aspas) — por isso a divisão da linha usa `limit = ncols`, que
    # deixa esse último campo intacto independente de quantas colunas vierem
    # antes dele.
    cabecalho = split(linhas[1], ',')
    idx = Dict(String(nome) => i for (i, nome) in enumerate(cabecalho))
    (haskey(idx, "method") && haskey(idx, "x")) ||
        error("Cabeçalho inesperado em $pontos_csv: $(linhas[1])")
    ncols = length(cabecalho)
    tem_fevals = haskey(idx, "function_evaluations")
    tem_rmsd = haskey(idx, "RMSD")

    pontos_por_metodo = Dict{String,Vector{Vector{Float64}}}()
    fevals_por_metodo = Dict{String,Vector{Union{Int,Missing}}}()
    rmsd_por_metodo = Dict{String,Vector{Float64}}()
    ordem_metodos = String[]

    for linha in linhas[2:end]
        isempty(strip(linha)) && continue
        campos = split(linha, ','; limit = ncols)
        length(campos) == ncols || error("Linha invalida em $pontos_csv: $linha")

        metodo = String(campos[idx["method"]])
        x_texto = strip(strip(campos[idx["x"]]), ['"'])
        x = parse.(Float64, split(strip(x_texto, ['[', ']']), ','))

        if !haskey(pontos_por_metodo, metodo)
            pontos_por_metodo[metodo] = Vector{Vector{Float64}}()
            tem_fevals && (fevals_por_metodo[metodo] = Union{Int,Missing}[])
            tem_rmsd && (rmsd_por_metodo[metodo] = Float64[])
            push!(ordem_metodos, metodo)
        end
        push!(pontos_por_metodo[metodo], x)

        if tem_fevals
            campo_fe = strip(campos[idx["function_evaluations"]])
            push!(fevals_por_metodo[metodo], isempty(campo_fe) ? missing : parse(Int, campo_fe))
        end
        tem_rmsd && push!(rmsd_por_metodo[metodo], parse(Float64, campos[idx["RMSD"]]))
    end

    return (;
        pontos_csv, pontos_por_metodo, metodos = ordem_metodos,
        fevals_por_metodo = tem_fevals ? fevals_por_metodo : nothing,
        rmsd_por_metodo = tem_rmsd ? rmsd_por_metodo : nothing,
    )
end
...
# ==============================================================================
# RMSD (y axis) against the cumulative number of function evaluations (x
# axis), one curve per method, from the enriched CSV written by
# `_comparar_solvers` (`sv_teste_pregenered.jl`): `method,point_index,
# function_evaluations,gradient_evaluations,RMSD,x`. `function_evaluations`
# and `RMSD` are already computed point by point in the CSV (see the comment
# above `_comparar_solvers`), so this function only reads and plots — no
# simulation is rerun. Points with no `function_evaluations` recorded (from a
# method that does not provide `accepted_function_evaluations`, see
# `_comparar_solvers`) are dropped before plotting that method.
# ==============================================================================

"""
    plot_rmsd_vs_evaluations(; points_csv="results/comparacao_solvers_real_dim10_pontos.csv",
                                 output="results/rmsd_vs_evaluations.pdf",
                                 yscale=:log10, plot_size=(700,500))

Plots, for each method present in `points_csv` (format `method,point_index,
function_evaluations,gradient_evaluations,RMSD,x` generated by
`_comparar_solvers`/`comparar_solvers_pregerado`/`comparar_solvers_real`,
`sv_teste_pregenered.jl`), the RMSD of each accepted point (y axis) against
the cumulative number of function evaluations up to that point (x axis) —
one curve per method, in the order the points were accepted. `yscale`
(default `:log10`, since RMSD typically drops several orders of magnitude)
and `plot_size` are passed on to `Plots.plot`. Uses the same colors/markers
as `plot_assimilacao_heatmap_tend_31_latex_com_solvers` (BFGS red/circle,
BOBYQA blue/diamond, ffjm2 green/triangle).

Raises an error if `points_csv` does not have the `function_evaluations`/
`RMSD` columns (old format, from before these columns existed — regenerate
the CSV by rerunning the corresponding comparison function). Saves the
figure to `output` and returns `(; plot, points_csv, output, curves)`, with
`curves` a `Vector` of `(; method, function_evaluations, rmsd)`.
"""
function plot_rmsd_vs_evaluations(;
    points_csv::AbstractString = "results/comparacao_solvers_real_dim10_pontos.csv",
    output::AbstractString = "results/rmsd_vs_evaluations.pdf",
    yscale::Symbol = :log10,
    xscale::Symbol = :identity,
    plot_size = (700, 500),
)
    data = le_pontos_aceitos_solvers(points_csv)
    data.fevals_por_metodo === nothing &&
        error("$points_csv has no function_evaluations column (old format); " *
              "regenerate the CSV by rerunning the corresponding comparison function.")
    data.rmsd_por_metodo === nothing &&
        error("$points_csv has no RMSD column (old format); " *
              "regenerate the CSV by rerunning the corresponding comparison function.")

    styles = Dict(
        "BFGS" => (color = :red, marker = :circle),
        "BOBYQA" => (color = :blue, marker = :diamond),
        "ffjm2" => (color = :green, marker = :utriangle),
    )
    extra_colors = [:orange, :purple, :brown, :gray]
    ...
    curves = NamedTuple[]
    for (j, method) in enumerate(data.metodos)
        raw_fevals = data.fevals_por_metodo[method]
        raw_rmsd = data.rmsd_por_metodo[method]
        valid = findall(!ismissing, raw_fevals)
        if isempty(valid)
            @warn "No function evaluations recorded for $method; skipping." points_csv
            continue
        end

        fevals = Int.(raw_fevals[valid])
        rmsd = raw_rmsd[valid]
        style = get(styles, method, (color = extra_colors[mod1(j, length(extra_colors))], marker = :hexagon))

        plot!(
            fig, fevals, rmsd;
            color = style.color, linewidth = 2, marker = style.marker, markersize = 4,
            markercolor = style.color, markerstrokecolor = :white, label = method,
        )

        push!(curves, (; method, function_evaluations = fevals, rmsd))
    end

    isempty(curves) && @warn "No curve was plotted." points_csv

    mkpath(dirname(output))
    savefig(fig, output)

    return (; plot = fig, points_csv, output, curves)
end
