...
# 1. Dependências
#
# Reaproveita de `ffjm2_box.jl` (que já inclui `ffjm2_trust.jl` e `ffjm2.jl`)
# o avaliador do modelo quártico, a resolução do subproblema restrito a uma
# caixa (`_ffjm2_trust_model_direction`) e a atualização quase-Newton por
# modelo (`_ffjm2_box_update!`, que já sabe fazer `:bfgs`, `:sr1`,
# `:is_bfgs`, `:dw_model`, `:is_dw_model` e `:psb`).
# ==============================================================================
...
if !isdefined(@__MODULE__, :_ffjm2_box_update!)
    include("ffjm2_box.jl")
end

const FFJM2_ENSEMBLE_MODELS = (:bfgs, :sr1, :is_bfgs, :dw_model, :is_dw_model, :psb)

# ==============================================================================
# 2. Método externo FFJM2 com conjunto (ensemble) de modelos quase-Newton
#
# Mantém um conjunto de Hessianas H[i] em paralelo, uma por modelo
# quase-Newton de `models` (todos definidos em `_ffjm2_box_update!`, ver
# `ffjm2_box.jl`). A cada iteração externa k>0:
#
...
#       de `ffjm2_box` (sem teste de razão, sem laço de encolher/resolver);
#   (d) o passo aceito atualiza todos os conjuntos de Hessianas, cada um com
#       sua própria fórmula, usando o mesmo par (s, y) — só existe uma
#       trajetória xₖ, então Δₖ = max(‖sₖ₋₁‖, trust_region_floor) também é
#       compartilhado entre os subproblemas de todos os modelos.
#
# Custa aproximadamente `length(models)` vezes o tempo de subproblema de
# `ffjm2_box` por iteração externa. É uma variante experimental para
# comparar o quanto os modelos concordam entre si (ver
# `last_model_directions` no retorno), não uma tentativa de ser mais rápida.
# ==============================================================================

"""
    ffjm2_ensemble(F, x0; models=FFJM2_ENSEMBLE_MODELS, jacobian=nothing, kwargs...)

Variante de [`ffjm2_box`](@ref) que, em vez de um único `update`, mantém um
conjunto `models` (por padrão os 6 modelos de `_ffjm2_box_update!`:
`:bfgs`, `:sr1`, `:is_bfgs`, `:dw_model`, `:is_dw_model`, `:psb`) de
Hessianas `H[i]` em paralelo. A cada iteração, o subproblema quártico em
caixa é resolvido uma vez por modelo, e a direção usada é a de maior
descida entre as candidatas, ou seja, a que minimiza ∇f(xₖ)ᵀd (mais
negativa).

A aceitação do passo (condição (2) do artigo + busca linear de Armijo, com
bola pesada como alternativa antes de cair em `-∇f(xₖ)`) e a definição de
`Δₖ = max(‖sₖ₋₁‖, trust_region_floor)` seguem exatamente [`ffjm2_box`](@ref).
Cada conjunto de Hessianas é atualizado com sua própria fórmula (via
`_ffjm2_box_update!`), usando o mesmo passo aceito e a mesma diferença de
Jacobiana para todos.

O retorno segue o mesmo formato de [`ffjm2_box`](@ref), exceto que
`hessians` é um `Dict{Symbol}` (uma entrada por modelo em vez de um único
vetor) e há um campo extra `last_model_directions` (`Dict{Symbol}` com a
última direção individual de cada modelo, antes da média — útil para
inspecionar o quanto os modelos concordam entre si).
"""
function ffjm2_ensemble(
    F,
    x0::AbstractVector;
    models::Tuple = FFJM2_ENSEMBLE_MODELS,
    jacobian = nothing,
    ...
    for k in 0:maxiter
        ...
        if k == 0
            ...
            for model in models
                ...
                if show_trace
                    println(
                        "  Subproblema ($model): Δ = $Δ | ", model_result.stop_reason,
        ...
        s = xnew - x
        for model in models
            _ffjm2_box_update!(H[model], s, Jnew, J, model, update_tol, gamma_bar)
        end
        previous_step = s
